...
const collectionHandlesToFetch = ['men', 'knives'];
...
`;
// const getAllProductsQuery = `
//   query getAllProducts($first: Int, $after: String, $collectionHandles: [String!]!) {
//     products(first: $first, after: $after, filters: { collections: $collectionHandles }) {
//       pageInfo {
//         hasNextPage
//         endCursor
//       }
...
const LandingPage = () => {
  ...
  const handleSearch = () => {
    console.log('collection Handles to Fetch', collectionHandlesToFetch);
    const filtered = products.filter((product) =>
      product.collections.edges.some(
        (collection) =>
          collectionHandlesToFetch.includes(collection.node.handle.toLowerCase()) &&
          (product.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            product.description.toLowerCase().includes(searchTerm.toLowerCase()))
      )
    );
